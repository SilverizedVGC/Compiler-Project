...
// Renamed from is_valid_identifier to avoid name collision with lexical-analyzer.cpp
bool is_identifier_type(const string& type) {
...
//value is the ACTUAL word, while type is just what kind of token it is 
    vector<Token> tokens = line_tokens; //  new token dynamic array to hold converted values, then return at the end of the function 
    
    int i = 0; //counter 
    int tcounter = 0; // for  t registers in assembly (likel identifiers )
    
    //KEYWORDS BLOCK 
    while(i < line_tokens.size()) { // roundabout way to check if there is a value stored at the token in the vector located at index i 
        if(line_tokens[i].type == "keyword") { // if it is a keyword
            ...
            if(line_tokens[i].value == "return") { // would the equivalent just be jmp back? for now, just translate to JMP command 
                ...
                if (i + 3 < line_tokens.size() && 
                   ((line_tokens[i+1].type == "punctuation" && line_tokens[i+2].type == "identifier" && line_tokens[i+3].type == "equals") || 
                    (line_tokens[i+1].type == "identifier" && line_tokens[i+2].type == "equals"))) {
                    // 1st is case with parentheses, 2nd is without, so just identifier and equals 
                    // 1st case is in the format:  if(a=)
                    tokens[i].value = "BEQ "; // branch if not equal, then later will scan for comparison variables 
                }
                else if (i + 3 < line_tokens.size() && 
                        ((line_tokens[i+1].type == "punctuation" && line_tokens[i+2].type == "identifier" && line_tokens[i+3].type == "not_equals") || 
                         (line_tokens[i+1].type == "identifier" && line_tokens[i+2].type == "not_equals"))) { // if the 
                    tokens[i].value = "BNE ";
                }
            }
        ...
        //OPERATOR BLOCK 
        // Add specific types like "addition", "subtraction", etc. to match Lexical Analyzer output
        else if(line_tokens[i].type == "operator" || line_tokens[i].type == "addition" || 
                line_tokens[i].type == "subtraction" || line_tokens[i].type == "multiplication") {
            ...
            //y = x + 5 
            if (i >= 3 && i + 1 < line_tokens.size() && 
                (line_tokens[i-1].type == "identifier" || line_tokens[i-1].type == "number") && 
                line_tokens[i-2].value == "=" && 
                (line_tokens[i-3].type == "identifier" || line_tokens[i-3].type == "number") &&
                (line_tokens[i+1].type == "identifier" || line_tokens[i+1].type == "number")) {
        ...
        //IDENTIFIER BLOCK 
        else if(is_identifier_type(line_tokens[i].type)) {
            // If an identifier follows a 'def' or 'class', it acts as a label.
            // Otherwise, it gets a register name (t0, t1, etc.)
            if (i > 0 && (line_tokens[i-1].value == "def" || line_tokens[i-1].value == "class")) {
                tokens[i].value = line_tokens[i].value + ": \n    ";
            } else {
                tokens[i].value = "t" + to_string(tcounter); 
                tcounter++;
            }
        }
